...
class Thim_Ekit_Widget_List_Event extends Widget_Base {
	...
	protected function _register_content_layout() {
		...
		$repeater->add_control(
			'custom_date_format',
			[
				'label'       => esc_html__( 'Custom Date Format', 'eduma' ),
				'type'        => Controls_Manager::TEXT,
				'placeholder' => 'F j, Y',
				'description' => sprintf(
					__( '<a href="%s">Documentation on date and time formatting.</a>', 'eduma' ),
...
//				$this->event_status      = get_post_meta( get_the_ID(), 'tp_event_status', true );
	...
	protected function render_date_end( $item ) {
		$date  = wpems_get_time( 'd' );
		$month = wpems_get_time( 'F' );
		if ( isset( $item['custom_date_format'] ) && !empty( $item['custom_date_format'] ) ) {
			echo '<div class="thim-ekits-event__date-month">';
			echo wpems_get_time( $item['custom_date_format'] );
			echo '</div>';
		} else {
