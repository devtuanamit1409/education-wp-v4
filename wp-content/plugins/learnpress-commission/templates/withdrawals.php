...
<?php } else {
        if( LearnPress::instance()->settings()->get( 'commission_enable_paypal_withdrawal_method' ) === 'yes') : ?>
            <?php foreach ( $withdrawal_methods as $method_key => $method_title ) { ?>
                <a class="lp-commission__button" href="javascript: showWithdrawalForm('<?php echo esc_attr( $method_key ); ?>');"><?php echo esc_html( $method_title ); ?></a>
            <?php } ?>
        <?php endif; ?>
	<?php foreach ( $withdrawal_methods as $method_key => $method_title ) { ?>
