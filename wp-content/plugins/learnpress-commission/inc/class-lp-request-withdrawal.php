...
if ( ! class_exists( 'LP_Request_Withdrawal' ) ) {
	...
	class LP_Request_Withdrawal {
		...
		private function init_hooks() {
			...
			// check conditional to display Commission Tab
			add_filter( 'learn-press/get-profile-tabs', array( $this, 'display_commission_tab' ), 10, 3 );
		}
		...
		public function profile_tabs( $tabs ) {
			...
			$tabs['withdrawals'] = array(
				...
				'icon'     => '<i class="fa fa-money-bill-alt"></i>',
			);

			return $tabs;
		}

		public function display_commission_tab( $tabs, $user_of_profile, $user_current ) {
			...
			$current_user_id = $user_current->get_id();
			$profile_user_id = $user_of_profile->get_id();
			// check when admin view user profile
		...
		private function new_withdrawal( $email, $value, $method, $user_id = null ) {
			...
			$now          = new LP_Datetime();
			$time_request = $now->format( LP_Datetime::I18N_FORMAT );
