...
const openPopupRazorpay = () => {
  ...
  button.addEventListener('click', e => {
    if (document.querySelector('#checkout-payment input.gateway-input:checked').value === 'razorpay') {
      // submit( button );
      if (!elCheckoutForm) {
      ...
      fetch(urlHandle, {
        ...
        if (data.result === 'processing') {
          ...
          RazorpayHandle.open();
          const elemPopup = document.querySelector('.razorpay-container iframe');
          if (elemPopup != null) {
            elemPopup.style.maxHeight = '640px';
          }
        } else {
          const elemError = document.querySelector('.learn-press-message.error');
          if (elemError != null) {
            elemError.remove();
          }
          elCheckoutForm.insertAdjacentHTML('afterbegin', `<div class="learn-press-message error">${data.messages}</div>`);
        }
